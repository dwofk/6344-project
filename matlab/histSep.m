...
png_path = fullfile(cd, 'exp_bracket_png');
...
jpg_path = fullfile(cd, 'exp_bracket_jpg');
if (exist(jpg_path, 'dir'))
    delete([jpg_path, '/*.jpg']);
else
    mkdir(jpg_path);
end
...
for bin=1:num_bins
    ...
    %out_jpg_filename = [jpg_path, '/exp', num2str(bin), '.jpg'];
    out_png_filename = [png_path, '/exp', num2str(bin), '.png'];
