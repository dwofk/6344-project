...
def runTrain(x,y,saver,loadModel, learning_rate_init, loadName='', saveName=''):
	...
	ave_loss_value=0
	    ...
	    im_in = cv2.cvtColor(im_in, cv2.COLOR_BGR2YCR_CB)
            h, w, channel = im_in.shape
            ...
            h, w, channel = im_la.shape
            im_la = cv2.cvtColor(im_la, cv2.COLOR_BGR2YCR_CB)
...
def processImage(modelName, imageName, savePath):
	...
	#im_out.show()
        ...
        im_out = im_out.reshape([h,w,channel]).clip(0,255).astype('uint8')
        im_out = cv2.cvtColor(im_out, cv2.COLOR_YCR_CB2BGR)
        cv2.imshow('image',im_out)
        cv2.waitKey(0)
        cv2.imwrite(savePath+'.jpg', im_out)
